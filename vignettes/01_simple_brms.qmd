...
::: {.callout-note}
## Question 1
We were interested if you are interested if if that sounds stimulation is where influence ATP C and which region
We were interested in question If the ITPC in the condition S=STIM is different between P and T condition.
:::
...
In the present study, we investigated whether the induced Inter-Trial Phase Coherence (ITPC) is correlated with 1/f slope of power spectrum of the signal. Question can be posed and answered in many different ways, using different methods. First I tried to use RBA (Gang Chen refernce) models looking for a difference between pairs of conditions then I generalized it to `brms` models.
