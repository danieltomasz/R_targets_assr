...
here::i_am(paste0(dir, "offset_work_june.qmd"))
#utils_env <- new.env()
...
spectral_offset_sigma <- brm(
  # Use bf() to specify distributional formulas
  bf(
    offset ~ 1 + S + (1 + S | subject) + (1 + S | roi),
  ...
  file = here::here(dir, "fits/spectral_offset_sigma"),
...
brain_plot_s_effect <- my_brain_plot(
  ...
  ggtitle(
    "Effect of Sound Stimulation on Spectral  Offset \n(Significant ROIs Only)"
  )
